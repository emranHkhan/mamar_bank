...
class WithdrawMoneyView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if self.request.user.account.is_bankrupt:
            messages.error(self.request, "The bank has gone bankrupt!")
...
class LoanRequestView(TransactionCreateMixin):
    ...
    def form_valid(self, form):
        ...
        if(current_loan_count >= 3):
            return HttpResponse("You have crossed yuo limit for loan.")

        messages.success(self.request, f"Loan request for {amount}$ is successful.")
...
class TransferMoneyView(View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            try:
                sender_info = get_object_or_404(UserBankAccount, account_no=self.request.user.account.account_no)
                receiver_info = get_object_or_404(UserBankAccount, account_no=receiver_account_no)
            ...
            Transaction.objects.create(
                account=sender_info,
                amount=amount,
                balance_after_transaction=sender_info.balance,
                transaction_type=5,  
                timestamp=timezone.now()
            )

            Transaction.objects.create(
                account=receiver_info,
                amount=amount,
                balance_after_transaction=receiver_info.balance,
                transaction_type=6, 
                timestamp=timezone.now()
            )
            send_transaction_email(self.request.user, amount, "Debit", "transactions/debit.html")
            send_transaction_email(receiver_info.user, amount, "Debit", "transactions/credit.html")
